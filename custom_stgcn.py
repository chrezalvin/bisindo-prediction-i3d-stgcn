from graph.custom_ntu_rgb_d import Graph

import tensorflow as tf
import numpy as np

from keras.layers import Conv3D, MaxPool3D, Conv2D, BatchNormalization, ReLU, MaxPooling2D, Flatten, Dense, Dropout, Activation, GlobalAveragePooling2D
from keras import Sequential, regularizers, initializers

REGULARIZER = regularizers.l2(0.0001)
INITIALIZER = initializers.VarianceScaling(scale=2.,
                            mode="fan_out",
...
class STGCN(tf.keras.Model):
    def __init__(self, filters, kernel_size=[9, 3], stride=1, activation='relu',
        ...
        self.tgcn = Sequential()
        self.tgcn.add(BatchNormalization(axis=1))
        self.tgcn.add(Activation(activation))
        self.tgcn.add(Conv2D(filters,
        ...
        if not residual:
            ...
            self.residual.add(Conv2D(filters,
                                kernel_size=[1, 1],
                                strides=[stride, 1],
                                padding='same',
                                kernel_initializer=INITIALIZER,
                                data_format='channels_first',
                                kernel_regularizer=REGULARIZER))
            self.residual.add(BatchNormalization(axis=1))
...
class Model(tf.keras.Model):
    def __init__(self, num_classes=60):
        ...
        self.pool = GlobalAveragePooling2D(data_format='channels_first')

        self.logits = Conv2D(num_classes,
                            kernel_size=1,
                            padding='same',
                            kernel_initializer=INITIALIZER,
                            data_format='channels_first',
